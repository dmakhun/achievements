...
<script type="text/javascript">
			...
			<table class="table">
				...
				<c:forEach var="item" items="${competencelist}">
					<tr id="competence-${item.id}">
						<td>${item.name}</td>
						<td><button id="button-${item.id}" class="btn btn-default"
								onclick="loadAchievementType('${item.id}');">Show
								achievement types</button></td>
					</tr>
				</c:forEach>
			</table>
		</div>
	...
	<div id="dialog" class="panel panel-default" style="display: none">
		<div class="panel-deading text-center">
			<h3>Add new achievement type</h3>
		</div>
				...
				<div class="col-md-12">
					<div class="form-group">
						<input class="form-control" id="achPoints" type="text" name="points"
							placeholder=<spring:message code ="achievement.points" /> />
					</div>
				</div>
			</div>
			...
			<div class="row text-center">
				<div class="col-md-12">
					<div class="btn-group btn-group-lg">
						<button id="achievement-type-submitter" type="button" class="btn btn-success" onclick="pass();">
