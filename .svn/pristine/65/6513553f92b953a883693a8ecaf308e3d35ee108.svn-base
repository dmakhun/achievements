	...
	<table class="table">
		...
		<c:forEach var="item" items="${competencelist}">
			<tr id="competence-${item.id}">
				<td>${item.name}</td>
				<td><button id="button-${item.id}" class="btn btn-default" onclick="loadAchievementType('${item.id}');">
...
<div id="dialog" style="display: none" title="Add new achievement type">
	<div class="panel-body">
		...
		<div class="row text-center">
			<div class="col-md-12">
				<div class="form-group">
					<button id="achievement-type-submitter" type="button" class="btn btn-success form-control" onclick="pass();">
